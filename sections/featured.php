<section class="py-10 bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-3 mb-6">
      <div>
        <h2 class="text-2xl sm:text-3xl font-extrabold text-slate-800 dark:text-slate-100 mb-1">Produk Unggulan</h2>
        <p class="text-slate-500 dark:text-slate-400 text-sm">Koleksi terbaik minggu ini yang dikurasi khusus untuk Anda.</p>
      </div>
      
      <a href="katalog.php" class="group flex items-center text-lime-600 dark:text-lime-400 font-bold hover:text-lime-700 dark:hover:text-lime-300 transition-colors duration-300 text-xs whitespace-nowrap bg-lime-50 dark:bg-lime-950/40 hover:bg-lime-100 dark:hover:bg-lime-950/60 px-3.5 py-1.5 rounded-lg border border-lime-100 dark:border-lime-900/50">
        Lihat Semua 
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <?php endwhile; else: ?>
        ...
        <div class="col-span-full py-10 text-center bg-white dark:bg-slate-900 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800">
          ...
          <p class="text-slate-500 dark:text-slate-400 text-sm font-medium">Belum ada produk yang tersedia saat ini.</p>
        </div>
