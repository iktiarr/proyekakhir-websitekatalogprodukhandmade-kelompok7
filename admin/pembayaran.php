<?php
// admin/pembayaran.php: Halaman kelola transaksi dan pembayaran untuk memverifikasi bukti transfer pembayaran dari pembeli.
...
?>
    ...
    <main class="flex-grow p-4 sm:p-6 w-full max-w-6xl mx-auto overflow-x-hidden">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div>
                <h1 class="text-xl sm:text-2xl font-extrabold text-slate-800 dark:text-slate-100 tracking-tight">
                    Verifikasi Pembayaran & Transaksi</h1>
                <p class="text-slate-500 dark:text-slate-400 text-xs mt-1">Kelola bukti pembayaran, status pengiriman
                    produk, dan transaksi pelanggan.</p>
            </div>
            <a href="cetak_laporan.php?tipe=keuangan" target="_blank"
                                ...
                                <tr
                                    ...
                                    <td class="px-4 py-3">
                                        <?php if ($baris['bukti_pembayaran']): ?>
                                            <span
                                                class="inline-flex items-center text-lime-700 dark:text-lime-400 font-bold text-[10px] bg-lime-50 dark:bg-lime-950/40 px-2 py-1 rounded-xl border border-lime-100 dark:border-lime-900/40 whitespace-nowrap">
                                                <i
                                                    class="fa-solid fa-file-invoice-dollar mr-1 text-lime-600 dark:text-lime-400"></i>
                                                <?= $baris['bukti_pembayaran']; ?>
                                            </span>
                                        <?php else: ?>
                                            <span
                                                class="text-slate-300 dark:text-slate-600 italic text-[11px] whitespace-nowrap">Belum
                                                Konfirmasi</span>
                                        <?php endif; ?>
                                    </td>
                                    <td class="px-4 py-3 text-center">
                                        <span
                                            class="inline-block px-2 py-0.5 rounded-xl text-[9px] font-bold uppercase tracking-wider border whitespace-nowrap <?= $warna_status[$baris['status']] ?? 'bg-slate-50 dark:bg-slate-900 text-slate-400 dark:text-slate-500 border-slate-200 dark:border-slate-800'; ?>">
                                            <?= $baris['status']; ?>
                                        </span>
                                    </td>
                                    <td class="px-4 py-3 pr-6 text-right">
                                        <div class="relative inline-block text-left group/menu">
                                            <button
                                                class="p-1.5 w-7 h-7 flex items-center justify-center rounded-xl text-slate-400 hover:text-slate-700 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors focus:outline-none cursor-pointer">
                                                <i class="fa-solid fa-ellipsis-vertical text-sm"></i>
                                            </button>

                                            <div
                                                class="absolute right-0 w-44 mt-1 bg-white dark:bg-slate-900 rounded-xl shadow-lg dark:shadow-none border border-slate-100 dark:border-slate-800 hidden group-hover/menu:block z-50 overflow-hidden transform origin-top-right">
                                                <div class="py-1 text-left">
                                                    <p
                                                        class="px-4 py-2 text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest border-b border-slate-50 dark:border-slate-800">
                                                        Aksi Pesanan</p>
                                                    <button type="button"
                                                    ...
                                                    <a href="pembayaran.php?id=<?= $baris['id']; ?>&status=dibayar"
                                                        class="block px-4 py-2 text-xs font-bold text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-950/30 transition-colors">
                                                        <i class="fa-solid fa-check w-4"></i> Konfirmasi Bayar
                                                    </a>
                                                    <a href="pembayaran.php?id=<?= $baris['id']; ?>&status=dikirim"
                                                        class="block px-4 py-2 text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950/30 transition-colors">
                                                        <i class="fa-solid fa-truck-fast w-4"></i> Kirim Produk
                                                    </a>
                                                    <a href="pembayaran.php?id=<?= $baris['id']; ?>&status=selesai"
                                                        class="block px-4 py-2 text-xs font-bold text-lime-600 dark:text-lime-400 hover:bg-lime-50 dark:hover:bg-lime-950/30 transition-colors">
                                                        <i class="fa-solid fa-flag-checkered w-4"></i> Selesaikan
                                                    </a>
                                                    <div class="h-px bg-slate-100 dark:bg-slate-800 my-1"></div>
                                                    <a href="pembayaran.php?id=<?= $baris['id']; ?>&status=dibatalkan"
                                                        class="block px-4 py-2 text-xs font-bold text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors">
        ...
        <div id="modal-detail-pesanan" class="fixed inset-0 z-50 overflow-y-auto hidden" aria-labelledby="modal-title"
            role="dialog" aria-modal="true">
            <!-- Backdrop Overlay -->
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div id="backdrop-modal"
                    class="fixed inset-0 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm transition-opacity duration-300 ease-out opacity-0">
                </div>
                ...
                <!-- Modal Container -->
                <div id="kontainer-modal"
                    class="inline-block align-bottom bg-white dark:bg-slate-900 rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full border border-slate-200 dark:border-slate-800 scale-95 opacity-0 duration-300 ease-out">

                    <!-- Header Modal -->
                    <div
                        class="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/50">
                        <div>
                            <h3
                                class="text-base font-extrabold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                                <i class="fa-solid fa-receipt text-lime-600 dark:text-lime-400"></i>
                                Rincian Transaksi <span id="md-tag"
                                    class="font-mono text-xs text-slate-400 dark:text-slate-500 font-bold"></span>
                            </h3>
                            <p class="text-[10px] text-slate-400 dark:text-slate-500 font-medium mt-0.5"
                                id="md-tanggal"></p>
                        </div>
                        <button type="button" id="tombol-tutup-modal"
                            class="p-2 rounded-xl text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all cursor-pointer flex items-center justify-center">
                    ...
                    <div class="p-6 space-y-6 max-h-[65vh] overflow-y-auto custom-scrollbar">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <!-- Pelanggan -->
                            <div
                                class="bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-start gap-3">
                                <div
                                    class="w-9 h-9 bg-lime-100 dark:bg-lime-950/40 text-lime-700 dark:text-lime-400 rounded-full flex items-center justify-center font-bold text-xs flex-shrink-0">
                                    <i class="fa-solid fa-user"></i>
                                </div>
                                <div class="min-w-0">
                                    <p
                                        class="text-[9px] text-slate-400 dark:text-slate-500 font-extrabold uppercase tracking-widest mb-0.5">
                                        Informasi Pelanggan</p>
                                    <p class="text-xs font-bold text-slate-800 dark:text-slate-100 truncate"
                                        id="md-nama"></p>
                                    <p class="text-[10px] text-slate-500 dark:text-slate-400 truncate" id="md-email">
                                    </p>
                                </div>
                            </div>
                            ...
                            <div
                                class="bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-start gap-3">
                                <div id="md-status-icon-container"
                                ...
                                <div>
                                    <p
                                        class="text-[9px] text-slate-400 dark:text-slate-500 font-extrabold uppercase tracking-widest mb-0.5">
                                        Status Pemesanan</p>
                                    <span id="md-status-badge"
                                        class="inline-block px-2.5 py-0.5 rounded-xl text-[9px] font-bold uppercase tracking-wider border mt-1"></span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            <h4
                                class="text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-3.5 flex items-center gap-1.5">
                                <i class="fa-solid fa-boxes-stacked text-lime-600 dark:text-lime-400"></i> Daftar Produk
                                Yang Dipesan
                            </h4>
                        ...
                        <!-- Alamat Pengiriman -->
                        <div
                            class="bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl border border-slate-100 dark:border-slate-800">
                            <h4
                                class="text-[9px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                        ...
                        <div
                            class="bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <p
                                    class="text-[9px] text-slate-400 dark:text-slate-500 font-extrabold uppercase tracking-widest mb-1">
                                    <i class="fa-solid fa-credit-card mr-1 text-[10px]"></i> Metode Pembayaran</p>
                                <p class="text-xs font-bold text-slate-800 dark:text-slate-200" id="md-metode"></p>
                            </div>
                            <div>
                                <p
                                    class="text-[9px] text-slate-400 dark:text-slate-500 font-extrabold uppercase tracking-widest mb-1">
                                    <i class="fa-solid fa-file-invoice-dollar mr-1 text-[10px]"></i> Bukti / Kode Bayar
                                </p>
                                <p class="text-xs font-mono font-bold text-lime-600 dark:text-lime-400" id="md-bukti">
                                </p>
                            </div>
                        </div>
                    ...
                    <div
                        class="px-6 py-4.5 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div class="text-center sm:text-left">
                            <p
                                class="text-[10px] text-slate-400 dark:text-slate-500 font-extrabold uppercase tracking-widest mb-0.5">
                                Total Harga Transaksi</p>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Membuka modal rincian pesanan dan mengisi data pesanan beserta daftar itemnya secara dinamis
            function bukaModalPesanan(btn) {
                ...
                // Mengisi list item yang dipesan ke dalam modal
                mdItemList.innerHTML = '';
                rincian.forEach(item => {
                    const imgSrc = item.gambar.startsWith('http') ? item.gambar : `../${item.gambar}`;
            ...
            // Menyembunyikan modal rincian pesanan dengan efek transisi
