...
if (isset($_POST['buat_pesanan'])) {
    ...
    try {
        ...
        foreach ($items as $item) {
            ...
            $prod_res = mysqli_fetch_assoc(mysqli_query($conn, "SELECT stok, nama_produk FROM produk WHERE id = $id_produk"));
            if ($prod_res['stok'] < $jumlah) {
                throw new Exception("Stok untuk produk '" . $prod_res['nama_produk'] . "' tidak mencukupi! Tersisa: " . $prod_res['stok']);
            }
...
<div class="py-8 bg-slate-50 dark:bg-slate-950 min-h-[75vh] transition-colors duration-300">
    <div class="max-w-6xl mx-auto px-4">
        ...
        <div class="mb-6">
            <span class="inline-block py-1 px-2.5 rounded-full bg-lime-100 dark:bg-lime-950/40 text-lime-700 dark:text-lime-400 text-[10px] font-bold tracking-wider mb-2 shadow-sm">
                TAHAP TERAKHIR
            </span>
            <h1 class="text-2xl font-extrabold text-slate-800 dark:text-slate-100">Checkout</h1>
        </div>

        <?php if (isset($error)): ?>
            <div class="bg-red-50 dark:bg-red-950/30 text-red-600 dark:text-red-400 p-4 rounded-xl mb-6 text-sm border border-red-100 dark:border-red-900/50 flex items-start gap-3">
        ...
        <form action="" method="POST" class="lg:flex lg:gap-6 items-start">
            ...
            <div class="lg:w-2/3 space-y-4 w-full mb-6 lg:mb-0">
                ...
                <div class="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
                    <h3 class="text-base font-bold text-slate-800 dark:text-slate-150 mb-4 flex items-center">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1">Nama Penerima</label>
                            <input type="text" value="<?= $_SESSION['nama']; ?>" readonly class="w-full px-3.5 py-2.5 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-500 dark:text-slate-400 outline-none cursor-not-allowed text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-600 dark:text-slate-450 mb-1">Nomor Telepon</label>
                            <input type="tel" name="no_telp" required class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-850 focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:ring-lime-500/20 focus:border-lime-500 outline-none transition-all duration-300 text-sm text-slate-800 dark:text-slate-200 placeholder-slate-400" placeholder="Contoh: 081234567890">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-600 dark:text-slate-450 mb-1">Alamat Lengkap</label>
                            <textarea name="alamat" required rows="3" class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-850 focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:ring-lime-500/20 focus:border-lime-500 outline-none transition-all duration-300 text-sm text-slate-800 dark:text-slate-200 placeholder-slate-400" placeholder="Contoh: Jl. Sudirman No. 123, RT 01/RW 02, Kel. Sukamaju, Kec. Maju, Kota Jakarta..."></textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        <label class="relative flex items-center p-3 border border-slate-200 dark:border-slate-800 rounded-xl cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-950/50 transition-colors has-[:checked]:border-lime-500 has-[:checked]:bg-lime-50/30 dark:has-[:checked]:bg-lime-950/20">
            ...
            <div class="lg:w-1/3 w-full">
                <div class="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm sticky top-24">
                    <h3 class="text-base font-bold text-slate-800 dark:text-slate-100 mb-4">Ringkasan Pesanan</h3>
                    ...
                    <div class="space-y-2 mb-4 max-h-40 overflow-y-auto pr-1 custom-scrollbar">
                        ...
                        while($row = mysqli_fetch_assoc($query_cart)): 
                        ?>
                        <div class="flex justify-between text-xs items-start gap-4 pb-2 border-b border-slate-50 dark:border-slate-800 last:border-0 last:pb-0">
                            <span class="text-slate-500 dark:text-slate-450 line-clamp-1">
                                <span class="font-bold text-slate-400 dark:text-slate-600"><?= $row['jumlah']; ?>x</span> <?= $row['nama_produk']; ?>
                            </span>
                            <span class="font-bold text-slate-700 dark:text-slate-300 whitespace-nowrap">Rp <?= number_format($row['harga'] * $row['jumlah'], 0, ',', '.'); ?></span>
                        </div>
                        <?php endwhile; ?>
                    </div>
                    ...
                    <div class="space-y-2.5 mb-6">
                        <div class="flex justify-between items-center pt-1.5">
                            ...
                            <span class="text-lg font-extrabold text-lime-600 dark:text-lime-400">Rp <?= number_format($total_harga, 0, ',', '.'); ?></span>
                        </div>
                    </div>
                    
                    <button type="submit" name="buat_pesanan" class="w-full bg-lime-600 text-white py-2.5 rounded-xl font-bold text-center flex items-center justify-center hover:bg-lime-700 hover:shadow-lg hover:shadow-lime-200/40 hover:-translate-y-0.5 transition-all duration-300 text-sm cursor-pointer border-none">
