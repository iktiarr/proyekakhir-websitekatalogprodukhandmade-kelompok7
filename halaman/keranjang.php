...
<div class="py-12 sm:py-16 bg-slate-50 dark:bg-slate-950 min-h-screen transition-colors duration-300">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        <div class="mb-8">
            <h1 class="text-3xl font-extrabold text-slate-800 dark:text-slate-100 tracking-tight">Keranjang Belanja</h1>
        </div>
            ...
            <form action="checkout.php" method="POST" id="formulirKeranjang">
                <div class="lg:flex lg:gap-8 items-start">
                    ...
                    <div class="lg:w-2/3 w-full mb-8 lg:mb-0">
                        <div class="bg-white dark:bg-slate-900 px-5 py-3.5 rounded-xl border border-slate-200/60 dark:border-slate-800 shadow-sm mb-4 flex items-center">
                            ...
                            <label for="pilihSemua" class="text-xs font-bold text-slate-750 dark:text-slate-300 cursor-pointer select-none">Pilih Semua Kerajinan</label>
                        </div>
                        ...
                        <div class="space-y-4">
                            ...
                            ?>
                            <div class="group bg-white dark:bg-slate-900 p-4 sm:p-5 rounded-xl border border-slate-200/60 dark:border-slate-800/80 shadow-sm flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-5 hover:shadow-md hover:border-lime-200 dark:hover:border-lime-700 transition-all duration-300 relative">
                                ...
                                <a href="keranjang.php?hapus=<?= $baris['id']; ?>" class="absolute top-4 right-4 sm:static sm:order-last text-slate-300 dark:text-slate-600 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 p-2.5 rounded-xl transition-colors duration-300 flex-shrink-0" title="Hapus Item">
                                    <i class="fa-solid fa-trash-can text-base"></i>
                                </a>

                                <div class="w-16 h-16 sm:w-20 sm:h-20 rounded-xl overflow-hidden bg-slate-50 dark:bg-slate-950 flex-shrink-0 border border-slate-150 dark:border-slate-855">
                                ...
                                <div class="flex-1 flex flex-col justify-center">
                                    ...
                                    <p class="text-lime-650 dark:text-lime-400 font-extrabold text-xs sm:text-sm mb-3 sm:mb-0">Rp <?= number_format($baris['harga'], 0, ',', '.'); ?></p>
                                ...
                                <div class="hidden sm:flex flex-col items-end gap-2 flex-shrink-0">
                                    <div class="flex items-center border border-slate-200 dark:border-slate-700 rounded bg-white dark:bg-slate-800 overflow-hidden focus-within:border-lime-500 focus-within:ring-2 focus-within:ring-lime-500/20 transition-all">
                                        <input type="number" name="jumlah[<?= $baris['id']; ?>]" value="<?= $baris['jumlah']; ?>" min="1" max="<?= $baris['stok']; ?>" class="w-12 text-center font-bold text-slate-850 dark:text-slate-200 bg-transparent border-none focus:ring-0 text-xs py-1 px-1.5 masukan-jumlah animate-none cursor-text" data-id="<?= $baris['id']; ?>">
                                    </div>
                                    <p class="text-slate-850 dark:text-slate-200 font-extrabold text-xs sm:text-sm nilai-total-sementara">Rp <?= number_format($total_sementara, 0, ',', '.'); ?></p>
                                </div>
                        ...
                        <div class="mt-6 flex flex-col-reverse sm:flex-row justify-between items-center gap-4">
                            <a href="katalog.php" class="group flex items-center text-slate-500 dark:text-slate-455 font-semibold hover:text-lime-650 dark:hover:text-lime-400 transition-colors duration-300 text-xs sm:text-sm">
                                <i class="fa-solid fa-arrow-left mr-2 transition-transform duration-300 group-hover:-translate-x-1"></i> Kembali ke Katalog
                            </a>
                            <button type="submit" name="update_cart" formaction="keranjang.php" class="w-full sm:w-auto bg-slate-100 dark:bg-slate-800 text-slate-650 dark:text-slate-300 px-5 py-3 rounded-xl font-bold hover:bg-lime-55 dark:hover:bg-lime-950/30 hover:text-lime-700 dark:hover:text-lime-400 hover:border-lime-200 border border-transparent transition-all duration-300 flex items-center justify-center text-xs sm:text-sm cursor-pointer shadow-sm">
                    ...
                    <div class="lg:w-1/3 w-full">
                        <div class="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-xl border border-slate-250/60 dark:border-slate-800 shadow-sm sticky top-24">
                            <h3 class="text-lg font-bold text-slate-850 dark:text-slate-100 mb-5">Ringkasan Pemesanan</h3>
                            
                            <div class="space-y-4 mb-6">
                                <div class="flex justify-between text-slate-550 dark:text-slate-450 text-xs sm:text-sm">
                                    <span>Total Kerajinan Pilihan</span>
                                    <span class="font-bold text-slate-800 dark:text-slate-250 nilai-total">Rp <?= number_format($total_akhir, 0, ',', '.'); ?></span>
                                </div>
                                <div class="flex justify-between text-slate-550 dark:text-slate-455 text-xs sm:text-sm">
                                    <span>Biaya Pengiriman</span>
                                    <span class="text-lime-700 dark:text-lime-400 font-extrabold bg-lime-50 dark:bg-lime-950/40 px-2.5 py-0.5 rounded-lg text-[10px] sm:text-xs border border-transparent dark:border-lime-900/30 shadow-sm">Gratis Ongkir</span>
                                </div>
                                ...
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="text-xl font-extrabold text-lime-600 dark:text-lime-400 nilai-total">Rp <?= number_format($total_akhir, 0, ',', '.'); ?></span>
                                </div>
                            </div>
                            
                            <button type="submit" name="checkout" class="group w-full inline-flex items-center justify-center bg-lime-600 text-white py-3.5 rounded-xl font-bold text-sm hover:bg-lime-700 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-lime-200/50 transition-all duration-300 cursor-pointer border-none outline-none">
                                Lanjutkan Checkout <i class="fa-solid fa-arrow-right ml-2 transition-transform duration-300 group-hover:translate-x-0.5"></i>
                            </button>
                        </div>
                    </div>
        ...
        <?php else: ?>
            ...
            <div class="py-16 sm:py-24 text-center bg-white dark:bg-slate-900 rounded-xl border border-dashed border-slate-200 dark:border-slate-800 shadow-sm">
                <div class="max-w-md mx-auto px-4">
                    <div class="w-16 h-16 bg-slate-50 dark:bg-slate-950 rounded-full flex items-center justify-center mx-auto mb-4.5 text-slate-300 dark:text-slate-700 text-3xl shadow-inner">
                    ...
                    <a href="katalog.php" class="inline-flex items-center justify-center bg-lime-600 text-white px-7 py-3.5 rounded-xl font-bold text-xs sm:text-sm hover:bg-lime-700 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-lime-200/50 transition-all duration-300">
