<section class="py-16 bg-white dark:bg-slate-900 transition-colors duration-300">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-8">
            <h2 class="text-3xl font-extrabold text-slate-800 dark:text-slate-100 mb-1">Apa Kata Kolektor?</h2>
            ...
            <?php if (isset($_SESSION['user_id'])): ?>
                <a href="<?= $awalan; ?>halaman/tambah_ulasan.php" class="inline-flex items-center gap-1.5 bg-lime-600 text-white px-5 py-2.5 rounded-xl font-bold text-xs hover:bg-lime-700 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-lime-200/40 transition-all duration-300">
                    <i class="fa-solid fa-pen-to-square"></i> Bagikan Ulasan Anda
                </a>
            <?php else: ?>
                <a href="<?= $awalan; ?>masuk.php" class="inline-flex items-center gap-1.5 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 px-5 py-2.5 rounded-xl font-bold text-xs hover:bg-slate-50 dark:hover:bg-slate-750 hover:border-slate-300 dark:hover:border-slate-650 hover:text-lime-600 dark:hover:text-lime-400 transition-all duration-300">
        ...
        ?>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <?php while($data_testimoni = mysqli_fetch_assoc($kueri_testimoni)): ?>
                    ...
                    <div class="bg-slate-50 dark:bg-slate-950 p-5 sm:p-6 rounded-2xl border border-slate-200/60 dark:border-slate-850 shadow-sm hover:shadow-xl hover:shadow-lime-900/5 hover:-translate-y-1 transition-all duration-300 flex flex-col relative group">
                        <div class="relative z-10 flex flex-col flex-grow">
                            ...
                            <div class="flex items-center gap-3 pt-3.5 border-t border-slate-200 dark:border-slate-800">
                                ...
                                <div>
                                    ...
                                    <p class="text-[10px] sm:text-xs text-slate-400 dark:text-slate-550">
        ...
        <?php else: ?>
            ...
            <div class="max-w-2xl mx-auto text-center py-12 bg-white dark:bg-slate-900 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800">
                ...
                <?php if (isset($_SESSION['user_id'])): ?>
                    <a href="<?= $awalan; ?>halaman/tambah_ulasan.php" class="inline-flex items-center gap-1.5 bg-lime-600 text-white px-5 py-2.5 rounded-xl font-bold text-xs hover:bg-lime-700 transition">
                        <i class="fa-solid fa-pen"></i> Tulis Ulasan Pertama
                    </a>
                <?php else: ?>
                    <a href="<?= $awalan; ?>masuk.php" class="inline-flex items-center gap-1.5 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 px-5 py-2.5 rounded-xl font-bold text-xs hover:bg-slate-50 dark:hover:bg-slate-750 hover:text-lime-650 dark:hover:text-lime-400 transition">
